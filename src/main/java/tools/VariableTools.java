                    ...
                    for(String simpleOperator:simpleOperators){
                        if(left.endsWith(simpleOperator))   // a*,=b;
                            res=left.replace(variable,"write_"+variable+"(read_"+variable+"()"
                                    +simpleOperator+"("+right.substring(1,right.length()-1)+"));");
                        else if(left.endsWith("<")||left.endsWith(">"))    // a<,=b
                            res = replaceRead(left, variable)+ right;
                        else {  // a,=b+1;
                            if(right.startsWith("=="))   res = replaceRead(left, variable)+ right;
                            else    res="write_"+variable+"("+right.substring(1,right.length()-1)+");";
                        }
                    }
                }else {
                    res=left+right;
